...
fromIndex=i+1;}}};class CyclicBuffer{#buffer;#head;#tail;#max;#full;constructor(size){if(typeof size==="number"){this.#max=size;this.#buffer=new Uint8Array(this.#max);}else{this.#max=size.length;this.#buffer=size;}
...
const HubCodes=['api_v','id','client','type','update','updates','get','last','crc32','discover','name','prefix','icon','PIN','version','max_upload','http_transfer','ota_type','ws_port','modules','total','used','code','OK','ack','info','controls','ui','files','notice','alert','push','script','refresh','print','error','fs_err','ota_next','ota_done','ota_err','fetch_start','fetch_chunk','fetch_err','upload_next','upload_done','upload_err','ota_url_err','ota_url_ok','value','maxlen','rows','regex','align','min','max','step','dec','unit','font_size','action','nolabel','suffix','notab','square','disable','hint','len','wwidth','wheight','data','wtype','keep','exp','plugin','js','css','ui_file','stream','port','canvas','width','height','active','html','dummy','menu','gauge','gauge_r','gauge_l','led','log','table','image','text','display','text_f','label','title','dpad','joy','flags','tabs','switch_t','switch_i','button','color','select','spinner','slider','datetime','date','time','confirm','prompt','area','pass','input','hook','row','col','space','platform','map','latlon','location','high_accuracy','layer','udp_port','container','rowcol','spoiler','http_port','tags','packet_id',];function decodeHubJson(data){if(!data||!data.length)return null;data=data.trim().replaceAll("#{","{").replaceAll("}#","}").replaceAll(/([^\\])\\([^\"\\nrt])/ig,"$1\\\\$2").replaceAll(/\t/ig,"\\t").replaceAll(/\n/ig,"\\n").replaceAll(/\r/ig,"\\r");for(const code in HubCodes){const re=new RegExp(`(#${Number(code).toString(16)})([:,\\]\\}])`,"ig");data=data.replaceAll(re,`"${HubCodes[code]}"$2`);}
const re=/(#[0-9a-f][0-9a-f])([:,\]\}])/ig;if(data.match(re))return null;else return data;}
...
await this.send(uri);}}
class MQTTConnection extends Connection{static priority=500;static name='MQTT';#client;#preflist;#buffers;constructor(hub){super(hub);this.options.enabled=false;this.options.host='test.mosquitto.org';this.options.port='8081';this.options.login='';this.options.password='';this.#preflist=[];this.#buffers=new Map();this.hub.addEventListener('deviceadded',ev=>this.#sub_device(ev.device.info.prefix,ev.device.info.id));}
isConnected(){return this.#client&&this.#client.connected;}
async discover(){if(this.isDiscovering()||!this.isConnected())return;for(const id of this.hub.getDeviceIds()){const dev=this.hub.dev(id);this._discoverTimer();await this.send(dev.info.prefix+'/'+dev.info.id+'='+this.hub.clientId);}}
async add(id){if(this.isDiscovering()||!this.isConnected())return;this._discoverTimer();await this.send(this.hub.prefix+'/'+id+'='+this.hub.clientId);}
async search(){if(this.isDiscovering()||!this.isConnected())return;await this.#upd_prefix(this.hub.prefix);this._discoverTimer();await this.send(this.hub.prefix+'='+this.hub.clientId);}
async connect(){await this.disconnect();this._setState(ConnectionState.CONNECTING);const url='wss://'+this.options.host+':'+this.options.port+'/mqtt';const options={keepalive:60,clientId:this.hub.clientId,username:this.options.login,password:this.options.password,protocolId:'MQTT',protocolVersion:4,clean:true,reconnectPeriod:3000,connectTimeout:10*1000}
try{this.#client=await mqtt.connectAsync(url,options);}catch(e){this._setState(ConnectionState.DISCONNECTED);return;}
this.#client.on('connect',()=>{this._setState(ConnectionState.CONNECTED);});this.#client.on('reconnect',()=>{this._setState(ConnectionState.CONNECTING);});this.#client.on('close',()=>{this._setState(ConnectionState.DISCONNECTED);});this.#client.on('error',()=>{this.disconnect();});this.#client.on('message',(topic,text)=>{topic=topic.toString();text=text.toString();const parts=topic.split('/');if(parts.length<2||parts[1]!='hub'||!this.#preflist.includes(parts[0]))
return;if(parts.length==2){this.hub._parsePacket(this,text);}else if(parts.length==4&&parts[2]==this.hub.clientId){let buffer=this.#buffers.get(parts[3]);if(!buffer){buffer=new PacketBufferScanFirst(data=>{this.hub._parsePacket(this,data);},1500);this.#buffers.set(parts[3],buffer);}
buffer.push(text);}else if(parts.length==5&&parts[3]=='get'){const dev=this.hub.dev(parts[2]);if(dev){let upd={};upd[parts[4]]={value:text};dev._checkUpdates(upd);}}});this.#preflist=[];await this.#upd_prefix(this.hub.prefix);for(const id of this.hub.getDeviceIds()){const dev=this.hub.dev(id);await this.#sub_device(dev.info.prefix,dev.info.id);}
this._setState(ConnectionState.CONNECTED);}
async disconnect(){if(this.#client){try{await this.#client.endAsync();}catch(e){}}
this.#client=undefined;this._setState(ConnectionState.DISCONNECTED);}
async send(topic){const i=topic.indexOf('=');let msg='';if(i!==-1){msg=topic.substring(i+1);topic=topic.substring(0,i);}
if(this.isConnected()){await this.#client.publishAsync(topic,msg);}}
async sub_device(prefix,id){await this.#sub_device(prefix,id);}
async#sub_device(prefix,id){if(!this.isConnected())return;await this.#client.subscribeAsync(prefix+'/hub/'+id+'/get/#');await this.#upd_prefix(prefix);}
async#upd_prefix(prefix){if(!this.isConnected()||this.#preflist.includes(prefix))return;this.#preflist.push(prefix);await this.#client.subscribeAsync(prefix+'/hub');await this.#client.subscribeAsync(prefix+'/hub/'+this.hub.clientId+'/#');}};class BLEConnection extends Connection{static priority=600;static name='BLE';#device;#characteristic;#buffer;#packet_buffer;constructor(hub){super(hub);this.options.enabled=false;this.options.buffer_size=1024;this.options.service_uuid=0xFFE0;this.options.characteristic_uuid=0xFFE1;this.options.max_size=20;this.options.max_retries=3;this.#packet_buffer=new PacketBufferScanAll(data=>{this.hub._parsePacket(this,data);});}
isConnected(){return this.options.enabled&&this.#device&&this.#device.gatt.connected&&this.#characteristic;}
getName(){return this.#device?this.#device.name:null;}
async begin(){this.#buffer=new CyclicBuffer(this.options.buffer_size);if(this.options.enabled)
await this.connect();}
async select(){await this.disconnect();if(this.#device){this.#device.removeEventListener('gattserverdisconnected',this._disconnect_h);this.#device=undefined;}
this._setState(ConnectionState.CONNECTING);try{this.#device=await navigator.bluetooth.requestDevice({filters:[{services:[this.options.service_uuid]}],});this.#device.addEventListener('gattserverdisconnected',this._disconnect_h);}finally{this._setState(ConnectionState.DISCONNECTED);}}
async connect(){if(!this.#device)
return;await this.disconnect();await this.#connect();}
async disconnect(){if(this.#characteristic){this.#characteristic.removeEventListener('characteristicvaluechanged',this._change_h);try{await this.#characteristic.stopNotifications();}catch(e){}
this.#characteristic=undefined;}
if(this.#device&&this.#device.gatt.connected){try{this.#device.gatt.disconnect();}catch(e){}}
this.#buffer.clear();this._setState(ConnectionState.DISCONNECTED);}
async send(data){data=new TextEncoder().encode(data);this.#buffer.push(data);while(this.isConnected()&&!this.#buffer.isEmpty()){const data=this.#buffer.pop(this.options.max_size);const device=this.#device;let retry=0;while(this.isConnected()&&device==this.#device){try{await this.#characteristic.writeValue(data);break;}catch(e){if(retry===this.options.max_retries)
throw e;await sleep(1);}}}}
async#connect(){this._setState(ConnectionState.CONNECTING);try{let server=this.#device.gatt;if(!server.connected){server=await server.connect();}
const service=await server.getPrimaryService(this.options.service_uuid);this.#characteristic=await service.getCharacteristic(this.options.characteristic_uuid);await this.#characteristic.startNotifications();this.#characteristic.addEventListener('characteristicvaluechanged',this._change_h);}catch(e){this._setState(ConnectionState.DISCONNECTED);throw e;}
this._setState(ConnectionState.CONNECTED);}
async _handleDisconnection(event){this._setState(ConnectionState.DISCONNECTED);await this.#connect();}
_disconnect_h=this._handleDisconnection.bind(this);async _btchanged(event){if(event.target.value)this.#packet_buffer.push(event.target.value);}
_change_h=this._btchanged.bind(this);}
class TelegramConnection extends Connection{static priority=400;static name='TG';#running;#offset;#buffers;constructor(hub){super(hub);this.options.enabled=false;this.options.token='';this.options.chat=0;this.options.discover_timeout=10000;this.#buffers=new Map();}
isConnected(){return this.#running;}
async connect(){if(!this.options.token||!this.options.chat)
return;this.#running=true;this.#offset=-1;this._setState(ConnectionState.CONNECTING);this.#poll();}
async disconnect(){this.#running=false;this._setState(ConnectionState.DISCONNECTED);}
async send(text){if(!this.options.chat)return;await this.#sendMessage(this.options.chat,'app:'+text)}
async#sendMessage(chat_id,text,params){params.chat_id=chat_id;params.text=text;params.disable_notification=true;const resp=await fetch(`https://api.telegram.org/bot${this.options.token}/sendMessage`,{method:'POST',body:JSON.stringify(params),headers:{'Content-Type':'application/json;charset=utf-8'},});const data=await resp.json();return data.ok;}
async#poll(){while(this.#running){let data=null;try{const resp=await fetch(`https://api.telegram.org/bot${this.options.token}/getUpdates?offset=${this.#offset}&timeout=${this.options.pool_time}`);data=await resp.json();}catch(e){}
if(!this.#running)break;if(data&&data.ok){this._setState(ConnectionState.CONNECTED);for(let upd of data.result){await this.#onmessage(upd);this.#offset=upd.update_id+1;}}else{this._setState(ConnectionState.DISCONNECTED);}
if(!this.#running)break;await sleep(this.options.pool_delay);}}
async#onmessage(data){try{if(data.channel_post.text=='/start'){const msg='Channel *'+data.channel_post.chat.title+'* id: `'+data.channel_post.chat.id+'`';await this.#sendMessage(data.channel_post.chat.id,msg,{parse_mode:"MarkdownV2"});return;}}catch(e){}
try{if(data.channel_post.chat.id!=this.options.chat)return;const[id,...rest]=data.channel_post.text.split(':');const data=rest.join(':');let buffer=this.#buffers.get(id);if(!buffer){buffer=new PacketBufferScanFirst(data=>{this.hub._parsePacket(this,data);},1500);this.#buffers.set(id,buffer);}
buffer.push(text);}catch(e){}}}
class SerialConnection extends Connection{static priority=900;static name='SERIAL';#port;#reader;#running;#packet_buffer;#writeLock;constructor(hub){super(hub);this.#writeLock=new AsyncLock();this.options.enabled=false;this.options.baudrate=115200;this.options.offset=2000;this.#packet_buffer=new PacketBufferScanAll(data=>{this.hub._parsePacket(this,data);});}
getName(){let id=this.#port?this.#port.getInfo().usbProductId:null;switch(id){case 0x7584:return'CH340S';case 0x55d3:return'CH343';case 0x7522:case 0x7523:return'CH340';case 0x5512:case 0x5523:case 0x5584:return'CH341';case 0x9500:case 0x0102:case 0x0501:case 0x80a9:case 0xea60:case 0xea61:case 0xea63:return'CP210x';case 0x0402:case 0x0403:case 0x0404:case 0x0405:case 0x6001:case 0x0602:case 0x6010:return'FT232';case null:return'none';default:return'unknown';}}
async begin(){try{const ports=await navigator.serial.getPorts();if(ports)
this.#port=ports[0];}catch(e){return;}
if(this.options.enabled)
await this.connect();}
async select(){await this.disconnect();this.#port=undefined;this._setState(ConnectionState.CONNECTING);try{const ports=await navigator.serial.getPorts();for(let port of ports)await port.forget();this.#port=await navigator.serial.requestPort();this.#port.addEventListener('disconnect',this._disconnect_h);}finally{this._setState(ConnectionState.DISCONNECTED);}}
async connect(){if(!this.#port)
return;await this.disconnect();this._setState(ConnectionState.CONNECTING);try{await this.#port.open({baudRate:this.options.baudrate});}catch(e){if(!(e instanceof InvaidStateError)){this._setState(ConnectionState.DISCONNECTED);throw e;}}
this._setState(ConnectionState.CONNECTED);this.#readLoop();}
async disconnect(){if(!this.#port)
return;this.#running=false;if(this.#reader)await this.#reader.cancel();try{await this.#port.close();}catch(e){}
this._setState(ConnectionState.DISCONNECTED);}
async send(data){data=new TextEncoder().encode(data+'\0');this.#writeLock.runExclusive(async()=>{if(!this.#port||!this.#port.writable)
return;const writer=this.#port.writable.getWriter();try{await writer.write(data);}finally{writer.releaseLock();}});}
async _handleDisconnection(event){this.disconnect();this.#port=undefined;}
_disconnect_h=this._handleDisconnection.bind(this);async#readLoop(){this.#running=true;while(this.#running&&this.#port&&this.#port.readable){this.#reader=this.#port.readable.getReader();try{while(true){const read=await this.#reader.read();if(read.done)break;if(read.value)this.#packet_buffer.push(read.value);}}finally{this.#reader.releaseLock();this.#reader=null;}}}}
...
async send(ip,port,uri){if(!port)port=this.options.port;let res=await http_get(`http://${ip}:${port}/hub/${uri}`,this.options.request_timeout);if(res.length&&res.startsWith('#{')&&res.endsWith('}#')){res=res.slice(2,-2);let chunks=res.split('}##{');for(let ch of chunks){await this.hub._parsePacket(this,'{'+ch+'}',ip,port);}}}};class WebSocketConnection extends Connection{static priority=800;static name='WS';#ws;#packet_buffer;#reconnect;onConnChange(s){}
...
async discover(){}
async search(){}
async connect(){if(this.#ws)return;this._setState(ConnectionState.CONNECTING);this.#reconnect=true;try{this.#ws=await WebSocketConnection.#wsOpenAsync(`ws://${this.options.ip}:${this.options.port}/`,['hub'],this.options.connect_timeout);this._setState(ConnectionState.CONNECTED);this.#packet_buffer.clear();this.#ws.onclose=async()=>{this._setState(ConnectionState.DISCONNECTED);this.#ws=undefined;await sleep(500);if(this.#reconnect)
await this.connect();};this.#ws.onmessage=(e)=>{this.#packet_buffer.push(e.data);};}catch(e){console.log(e);this._setState(ConnectionState.DISCONNECTED);}}
...
let ws;try{ws=new WebSocket(url,protos);}catch(e){console.log(e);return;}
ws.addEventListener('open',handler);ws.addEventListener('error',handler);})}}
...
class Device extends EventEmitter{active_connections=[];info;#input_queue;#pingTimer;prev_set={};skip_prd=1000;tout_prd=2500;object_tout=1500;get_tout=1500;ping_prd=3000;granted=false;cfg_flag=false;constructor(hub,id){super();this._hub=hub;this.info=hub.config.getDevice(id);this.#input_queue=new InputQueue(this.object_tout,this.get_tout);this.#pingTimer=new AsyncTimer(this.ping_prd,async()=>{try{await this.#postAndWait('ping',['OK','update','refresh','print']);this.dispatchEvent(new DeviceConnectionStatusEvent(this,true));}catch(e){console.log('[PING]',e);this.dispatchEvent(new DeviceConnectionStatusEvent(this,false));}
...
throw new DeviceError(HubErrors.Disabled);const data=await readFileAsArrayBuffer(file);const buffer=new Uint8Array(data);const crc=crc32(buffer);if(this.isHttpAccessable()&&this.info.http_transfer){let formData=new FormData();formData.append('upload',file,"upload");await http_post(`http://${this.info.ip}:${this.info.http_port}/hub/upload?path=${path}&crc32=${crc}&client_id=${this._hub.clientId}&size=${buffer.length}`,formData)}else{if(!progress)progress=()=>{};const upl_bytes=Array.from(buffer);const upl_size=upl_bytes.length;const max_enc_len=this.info.max_upload*3/4-60;let[cmd,data]=await this.#postAndWait('upload',['upload_next','upload_err'],path,upl_size);if(cmd==='upload_next')
...
throw new DeviceError(HubErrors.Disabled);if(this.isHttpAccessable()&&this.info.http_transfer){let formData=new FormData();formData.append(type,file,"ota");await http_post(`http://${this.info.ip}:${this.info.http_port}/hub/ota?type=${type}&client_id=${this._hub.clientId}`,formData)}else{if(!progress)progress=()=>{};const fdata=await readFileAsArrayBuffer(file);const buffer=new Uint8Array(fdata);const ota_bytes=Array.from(buffer);const ota_size=ota_bytes.length;const max_enc_len=this.info.max_upload*3/4-60;let[cmd,data]=await this.#postAndWait('ota',['ota_next','ota_done','ota_err'],type);while(cmd==='ota_next'){const data2=String.fromCharCode.apply(null,ota_bytes.splice(0,max_enc_len));progress(Math.round((ota_size-ota_bytes.length)/ota_size*100));[cmd,data]=await this.#postAndWait('ota_chunk',['ota_next','ota_done','ota_err'],(ota_bytes.length)?'next':'last',window.btoa(data2));}
if(cmd==='ota_err')
throw new DeviceError(data.code);}}};class SystemErrorEvent extends Event{constructor(text){super('systemerror');this.text=text;}}
class GyverHub extends EventEmitter{config;#connections=[];#devices=[];_ws;static api_v=2;constructor(){super();this.config=new Config();this.config.set('hub','prefix','MyDevices');this.config.set('hub','client_id',Math.round(Math.random()*0xffffffff+0xf0000000).toString(16));}
...
let checkData={ws_port:data.ws_port,http_port:data.http_port,udp_port:data.udp_port,http_transfer:data.http_transfer,}
for(const key in checkData){if(device.info[key]&&!checkData[key]){device.info[key]=0;infoChanged=true;}}
...
async _parsePacket(conn,data,ip=null,port=null){if(!data||!data.length)return;data=decodeHubJson(data);if(!data){this.dispatchEvent(new SystemErrorEvent('Device has newer API version. Update App!'));return;}
try{data=JSON.parse(data);}catch(e){console.log('Wrong packet (JSON): '+e+' in: '+data);this.dispatchEvent(new SystemErrorEvent('Wrong packet (JSON)'));return;}
if(!data.id){this.dispatchEvent(new SystemErrorEvent('Wrong packet (ID)'));return;}
if(data.client&&this.clientId!=data.client)return;const type=data.type;delete data.type;if(type=='discover'){if(!this.#isDiscovering()){console.log('Device not added (not discovering):',data);return;}
