...
send(ip,port,uri){http_get(`http://${ip}:${port}/${uri}`).then(res=>{if(res.length)this._hub._parsePacket(Conn.HTTP,res,ip,port);}).catch(e=>{});}
log(t){log('[HTTP] '+t);}
err(e){err('[HTTP] '+e);}};
...
devices=[];cfg={prefix:'MyDevices',client_id:new Date().getTime().toString(16).slice(-8),use_local:false,local_ip:'192.168.1.1',netmask:24,http_port:80,use_bt:false,use_serial:false,baudrate:115200,use_mqtt:false,mq_host:'test.mosquitto.org',mq_port:'8081',mq_login:'',mq_pass:'',api_ver:1};skip_prd=1000;tout_prd=2500;ping_prd=3000;constructor(){this.http=new HTTPconn(this);this.mqtt=new MQTTconn(this);this.serial=new SERIALconn(this);this.bt=new BTconn(this);}
