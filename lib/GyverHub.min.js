...
async select(){await this.close();const ports=await navigator.serial.getPorts();for(let port of ports)await port.forget();try{await navigator.serial.requestPort();await this.onportchange(true);}catch(e){this.onerror(e);await this.onportchange(false);}}
...
_port=null;_reader=null;_closing=false;async _readLoop(){this._closing=false;while(this._port.readable&&!this._closing){this._reader=this._port.readable.getReader();try{while(true){const read=await this._reader.read();if(read.done)break;const data=new TextDecoder().decode(read.value);this.onmessage(data);}}finally{this._reader.releaseLock();this._reader=null;}}}}
const Conn={SERIAL:0,BT:1,HTTP:2,MQTT:3,NONE:4,names:['Serial','BT','HTTP','MQTT','None'],};const Modules={INFO:(1<<0),FSBR:(1<<1),FORMAT:(1<<2),DOWNLOAD:(1<<3),UPLOAD:(1<<4),OTA:(1<<5),OTA_URL:(1<<6),REBOOT:(1<<7),SET:(1<<8),READ:(1<<9),DELETE:(1<<10),RENAME:(1<<11)};function http_get(url){return new Promise((res,rej)=>{try{var xhr=new XMLHttpRequest();xhr.onreadystatechange=function(){if(xhr.readyState==4){if(xhr.status==200)res(xhr.responseText);else rej("Error");}}
...
return ips;}
class PacketBuffer{_buf='';_start=false;constructor(hub,conn,byletter=false){this._hub=hub;this._conn=conn;this._byletter=byletter;}
process(data){if(this._byletter){for(let t of data){this._buf+=t;this.check();}}else{this._buf+=data;this.check();}}
check(){if(this._buf.startsWith('\n{'))this._start=true;if(this._buf.endsWith('}\n')){if(this._start)this._hub._parsePacket(this._conn,this._buf);this._start=false;this._buf='';}}};
...
this.bt.onerror=(e)=>this.err(e);this.bt.onmessage=(data)=>this.buf.process(data);}
...
async open(){if(!this.bt.state()){await this.bt.open();this.log('Connecting');this.onConnChange('connecting');}}
async close(){await this.bt.close();}
async send(text){await this.bt.send(text+'\0');}
log(t){this._hub.log('[BT] '+t);}
err(e){this._hub.err('[BT] '+e);}}
class HTTPconn extends Discover{tout_btw=10;tout=3000;constructor(hub){super(hub);}
...
log(t){log('[HTTP] '+t);}
err(e){err('[HTTP] '+e);}};
class MQTTconn extends Discover{tout=1000;onConnChange(state){}
...
class Device{tout_prd=2800;ping_prd=3000;constructor(hub){this._hub=hub;this.ws=new WSconn(this);this.mq_buf=new PacketBuffer(hub,Conn.MQTT);this.ws_buf=new PacketBuffer(hub,Conn.HTTP);}
info={id:'undefined',prefix:'undefined',name:'undefined',icon:'',PIN:0,version:'',max_upl:200,modules:0,ota_t:'.bin',ip:null,http_port:null,ws_port:81,};connected(){return!this.conn_lost;}
module(mod){return!(this.info.modules&mod);}
post(cmd,name='',value=''){cmd=cmd.toString();name=name.toString();value=value.toString();if(cmd=='set'){if(!this.module(Modules.SET))return;if(name)this.prev_set[name]={value:value,stamp:Date.now()};if(this.controls)this._updateControls(this.controls,name,'value',value);}
let uri0=this.info.prefix+'/'+this.info.id+'/'+this._hub.cfg.client_id+'/'+cmd;let uri=uri0;if(name){uri+='/'+name;if(value)uri+='='+value;}
switch(this.conn){case Conn.HTTP:if(this.ws.state())this.ws.send(uri);else this._hub.http.send(this.info.ip,this.info.http_port,`hub/${uri}`);break;case Conn.SERIAL:this._hub.serial.send(uri);break;case Conn.BT:this._hub.bt.send(uri);break;case Conn.MQTT:this._hub.mqtt.send(uri0+(name.length?('/'+name):''),value);break;}
if(this.focused){this._reset_ping();this._reset_tout();}}
focus(){this.focused=true;this.post('ui');if(this.conn==Conn.HTTP&&this.info.ws_port)this.ws.start();}
unfocus(){this.focused=false;this._stop_ping();this._stop_tout();this.post('unfocus');if(this.conn==Conn.HTTP)this.ws.stop();}
fsStop(){if(this.fs_mode)this.post('fs_abort');this._fsEnd();}
fsBusy(){return!!this.fs_mode;}
upload(file,path){if(this.fsBusy()){this._hub.onFsUploadError(this.info.id,'FS busy');return;}
let reader=new FileReader();reader.readAsArrayBuffer(file);reader.onload=(e)=>{if(!e.target.result)return;let buffer=new Uint8Array(e.target.result);if(!path.startsWith('/'))path='/'+path;if(!path.endsWith('/'))path+='/';path+=file.name;if(!confirm('Upload '+path+' ('+buffer.length+' bytes)?')){this._hub.onFsUploadError(this.info.id,'Upload cancelled');return;}
this._hub.onFsUploadStart(this.info.id);this.fs_mode='upload';if(this.conn==Conn.HTTP){let formData=new FormData();formData.append('upload',file);http_post(`http://${this.info.ip}:${this.info.http_port}/hub/upload?path=${path}`,formData).then((v)=>{this._hub.onFsUploadEnd(this.info.id,v);this._fsEnd();this.post('fsbr');}).catch((e)=>this._hub.onFsUploadError(this.info.id,e))}else{this.upl_bytes=Array.from(buffer);this.upl_size=this.upl_bytes.length;this.post('upload',path);}}}
uploadOta(file,type){if(this.fsBusy()){this._hub.onOtaError(this.info.id,'FS busy');return;}
if(!file.name.endsWith(this.info.ota_t)){alert('Wrong file! Use .'+this.info.ota_t);return;}
if(!confirm('Upload OTA '+type+'?'))return;let reader=new FileReader();reader.readAsArrayBuffer(file);reader.onload=(e)=>{if(!e.target.result)return;this._hub.onOtaStart(this.info.id);this.fs_mode='ota';if(this.conn==Conn.HTTP){let formData=new FormData();formData.append(type,file);http_post(`http://${this.info.ip}:${this.info.http_port}/hub/ota?type=${type}`,formData).then(()=>this._hub.onOtaEnd(this.info.id)).catch((e)=>this._hub.onOtaError(this.info.id,e)).finally(()=>this._fsEnd());}else{let buffer=new Uint8Array(e.target.result);this.upl_bytes=Array.from(buffer);this.upl_size=this.upl_bytes.length;this.post('ota',type);}}}
fetch(idx,path){let id=this.info.id;if(this.fsBusy()){this._hub.onFsFetchError(id,'FS busy');return;}
this.fs_mode='fetch';this.fet_name=path.split('/').pop();this.fet_index=idx;if(this.conn==Conn.HTTP){http_fetch_blob(`http://${this.info.ip}:${this.info.http_port}/hub/fetch?path=${path}`,perc=>this._hub.onFsFetchPerc(id,idx,perc)).then(res=>this._hub.onFsFetchEnd(id,this.fet_name,idx,res)).catch(e=>this._hub.onFsFetchError(id,idx,e)).finally(()=>this._fsEnd());}else{this.post('fetch',path);}
...
_updateControls(controls,name,key,value){for(let ctrl of controls){if(ctrl.type=='row'||ctrl.type=='col'){this._updateControls(ctrl.data,name,key,value);}else{if(ctrl.id==name)ctrl[key]=value;}}}
_fetchNextFile(){if(!this.files.length)return;let id=this.info.id;let file=this.files[0];if(this.fsBusy()){this._hub.onFetchError(id,file.name,file.data);return;}
this.fs_mode='fetch_file';this._hub.onFetchStart(id,file.name);if(this.conn==Conn.HTTP){http_fetch_blob(`http://${this.info.ip}:${this.info.http_port}/hub/fetch?path=${file.path}`,perc=>this._hub.onFetchPerc(id,file.name,perc)).then(res=>this._hub.onFetchEnd(id,file.name,file.data,`data:${getMime(file.path)};base64,${res}`)).catch(e=>this._hub.onFetchError(id,file.name,file.data)).finally(()=>{this._fsEnd();this._nextFile();});}else{post('fetch',file.path);}}
_fsEnd(){this.fs_mode=null;this._fsToutEnd();}
_nextFile(){this.files.shift();this._fetchNextFile();}
_checkUpdates(updates){for(let name in updates){let update=updates[name];for(let key in update){let value=update[key];if(key=='value'){if(this.prev_set[name]&&this.prev_set[name].value==value&&Date.now()-this.prev_set[name].stamp<this.tout_prd){delete update[key];continue;}}
this._updateControls(this.controls,name,key,value);}
this._hub.onUpdate(this.info.id,name,update);}}
_parse(type,data){let id=this.info.id;this._stop_tout();if(this.conn_lost){this.conn_lost=false;if(this.focused)this._hub.onDeviceConnChange(id,true);}
switch(type){case'OK':break;case'ui':this.controls=data.controls;break;case'update':this._checkUpdates(data.updates);break;case'upload_start':case'upload_next_chunk':if(this.fs_mode!='upload')break;this._uploadNextChunk();this._fsToutStart();break;case'upload_end':if(this.fs_mode!='upload')break;this._hub.onFsUploadEnd(id,'Upload done');this.post('fsbr');this._fsEnd();break;case'upload_err':if(this.fs_mode!='upload')break;this._hub.onFsUploadError(id,'Upload aborted');this._fsEnd();break;case'fetch_start':if(this.fs_mode!='fetch'&&this.fs_mode!='fetch_file')break;this.fet_buf='';this.post('fetch_chunk');this._fsToutStart();break;case'fetch_next_chunk':if(this.fs_mode!='fetch'&&this.fs_mode!='fetch_file')break;this.fet_buf+=data.data;if(data.chunk==data.amount-1){if(this.fs_mode=='fetch'){this._hub.onFsFetchEnd(id,this.fet_name,this.fet_index,this.fet_buf);this._fsEnd();}else{let file=this.files[0];this._hub.onFetchEnd(id,file.name,file.data,`data:${getMime(file.path)};base64,${this.fet_buf}`);this._fsEnd();this._nextFile();}}else{let perc=Math.round(data.chunk/data.amount*100);if(this.fs_mode=='fetch'){this._hub.onFsFetchPerc(id,this.fet_index,perc);}else{this._hub.onFetchPerc(id,this.files[0].name,perc);}
this.post('fetch_chunk');this._fsToutStart();}
break;case'fetch_err':if(this.fs_mode!='fetch'&&this.fs_mode!='fetch_file')break;if(this.fs_mode=='fetch'){this._hub.onFsFetchError(id,this.fet_index,'Fetch aborted');this._fsEnd();}else{this._hub.onFetchError(id,this.files[0].name,this.files[0].data);this._fsEnd();this._nextFile();}
break;case'ota_start':case'ota_next_chunk':if(this.fs_mode!='ota')break;this._otaNextChunk();this._fsToutStart();break;case'ota_end':if(this.fs_mode!='ota')break;this._hub.onOtaEnd(id);this._fsEnd();break;case'ota_err':if(this.fs_mode!='ota')break;this._hub.onOtaError(id,'OTA aborted');this._fsEnd();break;}}
_stop_tout(){if(this.tout){this._hub.onWaitAnswer(this.info.id,false);clearTimeout(this.tout);this.tout=null;}}
_reset_tout(){if(this.tout)return;this._hub.onWaitAnswer(this.info.id,true);this.tout=setTimeout(()=>{if(this.focused)this._hub.onDeviceConnChange(this.info.id,false);this.conn_lost=true;this._stop_tout();},this.tout_prd);}
_stop_ping(){if(this.ping)clearInterval(this.ping);this.ping=null;}
_reset_ping(){this._stop_ping();this.ping=setInterval(()=>{if(this.conn_lost)this._hub.onPingLost(this.info.id);else this.post('ping');},this.ping_prd);}
_otaNextChunk(){let i=0;let data='';while(true){if(!this.upl_bytes.length)break;i++;data+=String.fromCharCode(this.upl_bytes.shift());if(i>=this.info.max_upl*3/4)break;}
...
_uploadNextChunk(){let i=0;let data='';while(true){if(!this.upl_bytes.length)break;i++;data+=String.fromCharCode(this.upl_bytes.shift());if(i>=this.info.max_upl*3/4)break;}
...
_fsToutStart(){this._fsToutEnd();this.fs_tout=setTimeout(()=>{switch(this.fs_mode){case'upload':this._hub.onFsUploadError(this.info.id,'Upload timeout');break;case'fetch':this._hub.onFsFetchError(this.info.id,'Fetch timeout');break;case'fetch_file':this._hub.onFetchError(this.info.id,this.files[0].name,this.files[0].data);this._nextFile();break;case'ota':this._hub.onOtaError(this.info.id,'OTA timeout');break;}
this.fsStop();},this.tout_prd);}
...
conn=Conn.NONE;controls=null;granted=false;focused=false;tout=null;ping=null;conn_lost=false;prev_set={};fs_mode=null;fs_tout=null;upl_bytes=null;upl_size=null;fet_name='';fet_index=0;fet_buf='';files=[];file_flag=false;};
class GyverHub{onSaveDevices(){}
onAddDevice(dev){}
onUpdDevice(dev){}
onDiscoverEnd(){}
onDiscover(id,conn){}
...
devices=[];cfg={prefix:'MyDevices',client_id:new Date().getTime().toString(16).slice(-8),use_local:false,local_ip:'192.168.1.1',netmask:24,http_port:80,use_bt:false,use_serial:false,baudrate:115200,use_mqtt:false,mq_host:'test.mosquitto.org',mq_port:'8081',mq_login:'',mq_pass:'',};constructor(){this.http=new HTTPconn(this);this.mqtt=new MQTTconn(this);this.serial=new SERIALconn(this);this.bt=new BTconn(this);}
begin(){if(this.cfg.use_mqtt)this.mqtt.start();}
post(id,cmd,name='',value=''){this.dev(id).post(cmd,name,value);}
discover(){for(let dev of this.devices){dev.conn=Conn.NONE;}
if(this.cfg.use_mqtt)this.mqtt.discover();if(this.cfg.use_serial&&"serial"in navigator)this.serial.discover();if(this.cfg.use_bt&&"bluetooth"in navigator)this.bt.discover();if(this.cfg.use_local&&!isSSL())this.http.discover();this._checkDiscoverEnd();}
search(){if(this.cfg.use_mqtt)this.mqtt.search();if(this.cfg.use_serial&&"serial"in navigator)this.serial.search();if(this.cfg.use_bt&&"bluetooth"in navigator)this.bt.search();if(this.cfg.use_local&&!isSSL())this.http.search();this._checkDiscoverEnd();}
...
if(device.conn>conn){device.conn=conn;flag=true;}
...
err(e){console.log('Error: '+e.toString());}
_checkDiscoverEnd(){if(!this._discovering())this.onDiscoverEnd();}
_discovering(){return(this.http.discovering||this.mqtt.discovering||this.serial.discovering||this.bt.discovering);return this.http.discovering;}
...
return list;}
_parsePacket(conn,data,ip=null,port=null){data=data.trim().replaceAll(/([^\\])\\([^\"\\nrt])/ig,"$1\\\\$2").replaceAll(/\t/ig,"\\t").replaceAll(/\n/ig,"\\n").replaceAll(/\r/ig,"\\r");try{data=JSON.parse(data);}catch(e){this.err('Wrong packet (JSON): '+e+' in: '+data);return;}
if(!data.id)return this.err('Wrong packet (ID)');let type=data.type;delete data.type;if(type=='discover'&&this._discovering()){if(conn==Conn.HTTP){data.ip=ip;data.http_port=port;}
this.addDevice(data,conn);}
let device=this.dev(data.id);if(device){device._parse(type,data);let id=data.id;switch(type){case'error':this.onError(id,data.text);break;case'refresh':this.post(id,'ui');break;case'ack':this.onAck(id,data.name);break;case'fs_error':this.onFsError(id);break;case'info':this.onInfo(id,data.info);break;case'files':this.onFsbr(id,data.fs,data.total,data.used);break;case'print':this.onPrint(id,data.text,data.color);break;case'discover':this.onDiscover(id,conn);break;case'ui':this.onUi(id,data.controls,conn,device.info.ip);break;case'data':this.onData(id,data.data);break;case'alert':this.onAlert(id,data.text);break;case'notice':this.onNotice(id,data.text,intToCol(data.color));break;case'push':this.onPush(id,data.text);break;case'ota_url_ok':this.onOtaUrlEnd(id);break;case'ota_url_err':this.onOtaUrlError(id);break;}}}};
