...
info={id:'undefined',prefix:'undefined',name:'undefined',icon:'',PIN:0,version:'',max_upl:200,modules:0,ota_t:'.bin',ip:null,http_port:null,ws_port:81,http_t:1,api_v:0,};connected(){return!this.conn_lost;}
...
upload(file,path){if(!this.module(Modules.UPLOAD))return;if(this.fsBusy()){this._hub.onFsUploadError(this.info.id,HubErrors.FsBusy);return;}
let reader=new FileReader();reader.readAsArrayBuffer(file);reader.onload=(e)=>{if(!e.target.result)return;let buffer=new Uint8Array(e.target.result);if(!path.startsWith('/'))path='/'+path;if(!path.endsWith('/'))path+='/';path+=file.name;if(!confirm('Upload '+path+' ('+buffer.length+' bytes)?')){this._hub.onFsUploadError(this.info.id,HubErrors.Cancelled);return;}
...
uploadOta(file,type){if(!this.module(Modules.OTA))return;if(this.fsBusy()){this._hub.onOtaError(this.info.id,HubErrors.FsBusy);return;}
...
_checkUpdates(updates){for(let name in updates){if(name.includes(';')){name.split(';').forEach(n=>updates[n]=updates[name]);delete updates[name];}}
...
if(this.cfg.use_mqtt)this.mqtt.discover();if(this.cfg.use_tg)this.tg.discover();if(this.cfg.use_serial&&"serial"in navigator)this.serial.discover();if(this.cfg.use_bt&&"bluetooth"in navigator)this.bt.discover();if(this.cfg.use_local&&!isSSL())this.http.discover();this._checkDiscoverEnd();}
search(){if(this.cfg.use_mqtt)this.mqtt.search();if(this.cfg.use_tg)this.tg.search();if(this.cfg.use_serial&&"serial"in navigator)this.serial.search();if(this.cfg.use_bt&&"bluetooth"in navigator)this.bt.search();if(this.cfg.use_local&&!isSSL())this.http.search();this._checkDiscoverEnd();}
