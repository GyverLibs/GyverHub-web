...
async getPorts(){return await navigator.serial.getPorts();}
async select(){await this.close();const ports=await this.getPorts();for(let port of ports)await port.forget();try{await navigator.serial.requestPort();await this.onportchange(true);}catch(e){this.onerror(e);await this.onportchange(false);}}
async open(baud){try{if(this._port)throw"Already open";const ports=await this.getPorts();if(!ports.length)throw"No port";this._port=ports[0];try{await this._port.open({baudRate:baud});this.onopen();await this._readLoop();}finally{await this._port.close();this._port=null;this.onclose();}}catch(e){this.onerror(e);}}
...
async getPorts(){return await this.ser.getPorts();}
...
post(cmd,name='',value=''){cmd=cmd.toString();name=name.toString();value=value.toString();if(cmd=='set'){if(!this.module(Modules.SET))return;if(name)this.prev_set[name]={value:value,stamp:Date.now()};this._updateControls(this.controls,name,'value',value);}
...
upload(file,path){if(!this.module(Modules.UPLOAD))return;if(this.fsBusy()){this._hub.onFsUploadError(this.info.id,'FS busy');return;}
...
fetch(idx,path){if(!this.module(Modules.FETCH))return;let id=this.info.id;if(this.fsBusy()){this._hub.onFsFetchError(id,idx,'FS busy');return;}
...
_updateControls(controls,name,key,value){if(!this.controls)return;for(let ctrl of controls){if(ctrl.type=='row'||ctrl.type=='col'){this._updateControls(ctrl.data,name,key,value);}else{if(ctrl.id==name)ctrl[key]=value;}}}
...
switch(type){case'OK':break;case'ui':this.controls=data.controls;break;case'update':this._checkUpdates(data.updates);break;case'fetch_start':if(this.fs_mode!='fetch'&&this.fs_mode!='fetch_file')break;this.fet_len=data.len;this.fet_buf='';this.post('fetch_next');this._fsToutStart();if(this.fs_mode=='fetch'){this._hub.onFsFetchPerc(id,this.fet_index,0);}else{this._hub.onFetchPerc(id,this.files[0].name,0);}
break;case'fetch_chunk':if((this.fs_mode!='fetch'&&this.fs_mode!='fetch_file')||this.fet_buf==null)break;this.fet_buf+=atob(data.data);if(data.last){if(this.fet_buf.length==this.fet_len){let b64=btoa(this.fet_buf);if(this.fs_mode=='fetch'){this._hub.onFsFetchEnd(id,this.fet_name,this.fet_index,b64);this._fsEnd();}else{let file=this.files[0];this._hub.onFetchEnd(id,file.name,file.data,`data:${getMime(file.path)};base64,${b64}`);this._fsEnd();this._nextFile();}}
this.fet_buf=null;}else{let perc=Math.round(this.fet_buf.length/this.fet_len*100);if(this.fs_mode=='fetch'){this._hub.onFsFetchPerc(id,this.fet_index,perc);}else{this._hub.onFetchPerc(id,this.files[0].name,perc);}
this.post('fetch_next');this._fsToutStart();}
break;case'fetch_err':if(this.fs_mode!='fetch'&&this.fs_mode!='fetch_file')break;this.fet_buf=null;if(this.fs_mode=='fetch'){this._hub.onFsFetchError(id,this.fet_index,'Fetch error: '+data.text);this._fsEnd();}else{this._hub.onFetchError(id,this.files[0].name,this.files[0].data);this._fsEnd();this._nextFile();}
break;case'upload_next':if(this.fs_mode!='upload')break;this._uploadNextChunk();this._fsToutStart();break;case'upload_done':if(this.fs_mode!='upload')break;this._hub.onFsUploadEnd(id,'Upload done');this._fsEnd();this.post('files');break;case'upload_err':if(this.fs_mode!='upload')break;this._hub.onFsUploadError(id,'Upload error: '+data.text);this._fsEnd();break;case'ota_next':if(this.fs_mode!='ota')break;this._otaNextChunk();this._fsToutStart();break;case'ota_done':if(this.fs_mode!='ota')break;this._hub.onOtaEnd(id);this._fsEnd();break;case'ota_err':if(this.fs_mode!='ota')break;this._hub.onOtaError(id,'OTA error: '+data.text);this._fsEnd();break;}}
...
_fsToutStart(){this._fsToutEnd();this.fs_tout=setTimeout(()=>{switch(this.fs_mode){case'upload':this._hub.onFsUploadError(this.info.id,'Upload timeout');break;case'fetch':this._hub.onFsFetchError(this.info.id,this.fet_index,'Fetch timeout');this.fet_buf=null;break;case'fetch_file':this._hub.onFetchError(this.info.id,this.files[0].name,this.files[0].data);this.fet_buf=null;this._nextFile();break;case'ota':this._hub.onOtaError(this.info.id,'OTA timeout');break;}
...
conn=Conn.NONE;conn_arr=[0,0,0,0];controls=null;granted=false;focused=false;tout=null;ping=null;conn_lost=false;prev_set={};fs_mode=null;fs_tout=null;upl_bytes=null;upl_size=null;fet_name='';fet_index=0;fet_buf=null;fet_len=0;files=[];file_flag=false;};
